...
disp_file = open('text_files/AEM6318_crane_bar_displacement.txt','r')
reactions_file = open('text_files/AEM6318_crane_bar_reactions.txt','r')
...
nodes_disp_coord_scaled = np.zeros((n_nodes,3))
U = np.zeros((n_nodes,3))
nodes_reactions = np.zeros((n_nodes,4))
...
strain_file = open('text_files/AEM6318_crane_bar_strains.txt','r')
stress_file = open('text_files/AEM6318_crane_bar_stress.txt','r')
...
strain_file.close()
stress_file.close()
elements = list((elements_id , elements_nodes , elements_area , elements_elasticity , elements_strain, elements_stress))
...
for i in range(len(elements[0])):
    if elements[2][i] == elements_area[0]:
    ...
    elif elements[2][i] == elements_area[1]:
        ax.plot(xs=[nodes[1][int(elements[1][i,0]-1),0], nodes[1][int(elements[1][i,1]-1),0]], \
                ys=[nodes[1][int(elements[1][i,0]-1),1], nodes[1][int(elements[1][i,1]-1),1]], \
                zs=[nodes[1][int(elements[1][i,0]-1),2], nodes[1][int(elements[1][i,1]-1),2]], color = 'salmon', linewidth = .25)
        ax.plot(xs=[nodes[2][int(elements[1][i,0]-1),0], nodes[2][int(elements[1][i,1]-1),0]], \
                ys=[nodes[2][int(elements[1][i,0]-1),1], nodes[2][int(elements[1][i,1]-1),1]], \
                zs=[nodes[2][int(elements[1][i,0]-1),2], nodes[2][int(elements[1][i,1]-1),2]], color = 'red', linewidth = 0.5)
...
plt.savefig('Undeformed_Deformed_Crane.pdf',format="pdf")
plt.show()
...
for i in range(len(elements[0])):
    if elements[2][i] == elements_area[0]:
        ax.plot(xs=[nodes[2][int(elements[1][i,0]-1),0], nodes[2][int(elements[1][i,1]-1),0]], \
                ys=[nodes[2][int(elements[1][i,0]-1),1], nodes[2][int(elements[1][i,1]-1),1]], \
                zs=[nodes[2][int(elements[1][i,0]-1),2], nodes[2][int(elements[1][i,1]-1),2]], color = 'c', linewidth = 1)

    elif elements[2][i] == elements_area[1]:
        ax.plot(xs=[nodes[2][int(elements[1][i,0]-1),0], nodes[2][int(elements[1][i,1]-1),0]], \
                ys=[nodes[2][int(elements[1][i,0]-1),1], nodes[2][int(elements[1][i,1]-1),1]], \
                zs=[nodes[2][int(elements[1][i,0]-1),2], nodes[2][int(elements[1][i,1]-1),2]], color = 'y', linewidth = 0.5)
j = 0
for j in range(n_nodes):
    ax.scatter(nodes[2][j,0],nodes[2][j,1],nodes[2][j,2],s=4, color = 'grey')
...
for i in range(len(nodes[0])):
    ...
    if abs(nodes[5][i,3]) > 1e-5:
        coordR = nodes[2][i,:]
        ax.quiver(coordR[0],coordR[1],coordR[2],nodes[5][i,0],nodes[5][i,1],nodes[5][i,2], length=0.03, color = 'black')
...
plt.xlabel("x")
plt.ylabel("y")
plt.savefig('Forces_Reactions_Crane.pdf',format="pdf")
plt.show()
...
for i in range(len(elements[0])):
    if elements[2][i] == elements_area[0]:
        ...
        strainRange = (elements[4][i]-min(elements[4]))/(max(elements[4])-min(elements[4]))
        ax.plot(xs, ys, zs, color = plt.cm.jet(strainRange), linewidth = 2)
...
norm = mpl.colors.Normalize(min(elements[4]), max(elements[4]))
...
plt.savefig('Strains_Crane.pdf',format="pdf")
plt.show()
...
for i in range(len(elements[0])):
    if elements[2][i] == elements_area[0]:
        ...
        stressRange = (elements[5][i]-min(elements[5]))/(max(elements[5])-min(elements[5]))
        ax.plot(xs, ys, zs, color = plt.cm.jet(stressRange), linewidth = 2)
...
plt.savefig('Stress_Crane.pdf',format="pdf")
plt.show()
...
Ux = U[:,0:1]
Ux_mat = np.concatenate((Ux, np.zeros((len(nodes[0]),2))),1)
...
for i in range(len(elements[0])):
    if Ux[int(elements[1][i,0]-1)] > Ux[int(elements[1][i,1]-1)]:
        ...
        for j in range(lin-1):
            if elements[2][i] == elements_area[0]:
                ax.plot(xs[j:j+2], ys[j:j+2], zs[j:j+2], color=plt.cm.jet(grad[j]), linewidth = 1.5)
                
            elif elements[2][i] == elements_area[1]:
                ax.plot(xs[j:j+2], ys[j:j+2], zs[j:j+2], color=plt.cm.jet(grad[j]), linewidth = .5)
...
plt.savefig('x_Displacement_Crane.pdf',format="pdf")
plt.show()
...
Uy = U[:,1:2]
Uy_mat = np.concatenate((np.zeros((len(nodes[0]),1)), Uy, np.zeros((len(nodes[0]),1))),1)
...
plt.savefig('y_Displacement_Crane.pdf',format="pdf")
plt.show()
...
Uz = -U[:,2:3]
Uz_mat = np.concatenate((np.zeros((len(nodes[0]),2)), Uz),1)
...
plt.savefig('z_Displacement_Crane.pdf',format="pdf")
plt.show()
...
plt.savefig('Total_Displacement_Crane.pdf',format="pdf")
plt.show()
